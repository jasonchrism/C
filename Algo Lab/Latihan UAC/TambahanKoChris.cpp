...
int search(char* playlistName){
    ...
    while (left <= right) { // usahain <= aja
        ...
        if (strcmp(pl[mid].name, playlistName) == 0) { //yang dicek yang di KANAN
            return mid;
        } else if (strcmp(pl[mid].name, playlistName) > 0)  {
            right = mid - 1;
        } else {
            left = mid + 1;
        }
...
int partitionSong(int low, int high, int index){
    ...
    int j = low;
    char pivot[200];
    int songIndex = pl[index].songCount - 1;
    strcpy(pivot, pl[index].song[songIndex].title);
...
void addMenu(){
    ...
    do {
        printf("Please input the duration [in seconds]: ");
        scanf("%d", &duration);
        getchar();
    } while (duration < 0);
    do {
...
void searchMenu(){
    ...
    for (int i = 0; i < plCount; i++) {
        ...
        for (int j = 0; j < pl[i].songCount; j++) {
            char lwr1[200] = {};
            strcpy(lwr1, pl[i].song[j].title);
            if (strstr(strlwr(lwr1), strlwr(check)) != NULL) {
                if (x++ == 0) printf("%s\n", pl[i].name);
                printf("\t%s - %s - %d\n", pl[i].song[j].title, pl[i].song[j].artist, pl[i].song[j].duration);
            }
        }
